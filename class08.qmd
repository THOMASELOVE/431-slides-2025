...
format:
  revealjs: 
    ...
    footer: "431 Class 08 | 2025-09-25 | <https://thomaselove.github.io/431-2025/>"
---
...
#| echo: true
ohio_8 <- oh19 |>
  select(code, county, h_out24, h_fac19rk) |>
...
Aassumes you need no adjustment for the fact that you are doing multiple comparisons at once.
...
Uniformly more powerful than Bonferroni, while preserving the overall false positive rate at $\alpha$. 
...
con_holm <- estimate_contrasts(fit1, ci = 0.99, p_adjust = "holm")
...
set.seed(431)
fit2 <- lm(h_out24 ~ hfactor, 
           data = ohio_8, refresh = 0)

model_parameters(fit2) 
```

## Holm 99% CIs after Bayes Fit

```{r}
#| echo: true

estimate_contrasts(fit2, ci = 0.99, 
                   p_adjust = "Holm")
```
...
con_bay <- as_tibble(
    estimate_contrasts(fit2, ci = 0.99, p_adjust = "Holm")) |>
    mutate(contr = str_c(Level1, " - ", Level2))

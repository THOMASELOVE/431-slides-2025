...
format:
  revealjs: 
    ...
    footer: "431 Class 18 | 2025-11-04 | <https://thomaselove.github.io/431-2025/>"
---
...
```{r}
#| echo: true
model_parameters(mainfx, ci = 0.90, pretty_names = FALSE)
```
...
- If `dep_start` = 0, High `ed_start` yields 33.4; Low yields 34.1, so High - Low difference in estimates is -0.7.
- If `dep_start` = 1, High `ed_start` yields 35.7; Low yields 38.6, so High - Low difference in estimates is -2.9.
...
```{r}
#| echo: true
model_parameters(mainfx2, ci = 0.90, pretty_names = FALSE)
```
...
```{r}
#| echo: true
model_parameters(withint2, ci = 0.90, pretty_names = FALSE)
```
